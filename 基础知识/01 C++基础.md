...
dequeu —— 双端队列（数组+链表结合，同vector导致迭代器失效）   
string  
...
```c
1) 借助std::remove()函数
c.erase(remove(c.begin(), c.end(), value), c.end()) // remove()将==value的值都放到后面，但是不删除
c.erase(remove_if(c.begin(), c.end(), function), c.end()) // remove_if() value替换成function
2) 使用迭代器
