...
### 5.类内成员变量初始化  
static int c 必须在类外初始化是因为static只能被初始化一次，若在类内，每次创建实例时并不清楚是否已经被初始化过一次  
static const int x 可以在类内是因为已经有const修饰了，初始化一次后不会再被修改  
```c
class Test
{
public:
...
## STL
### 序列式容器 
vector/list/queue/priority_queue/stack  
### 关联式容器  
map/set/multimap/multiset : 底层机制为BR-Tree
unordered_map ：底层机制为hashtable
### 2.STL释放内存
1.含有reserve()/capacity()的容器才需要使用swap释放空间，仅有vector和string符合，其余无效  
